...
import am.basic.jdbcStart.repository.UsersNotesRepository;
...
public class UserService {
    ...
    private UsersNotesRepository usersNotesRepository = new UsersNotesRepository(new DataSource());
    ...
    public void saveNoteInBase(String username, String note) throws NotFoundException, InternalServerException {
        try {
            User user = userRepository.getByUsername(username);
            NotFoundException.check(user == null, INVALID_CREDENTIALS_MESSAGE);
            usersNotesRepository.add(note,user.getId());
        } catch (SQLException e) {
            e.printStackTrace();
            throw new InternalServerException(INTERNAL_ERROR_MESSAGE);
        }
